...
- **D2 — Orçamento de pesquisa. DECIDIDO: por tempo.** Cada prompt-task define X minutos **de experimentação apenas** (setup, testes e fechamento ficam fora do relógio; limite brando; teto de 25 experimentos por run); regras em [07 §6](07_agent_protocol.md). Salvaguardas mantidas como propostas (a confirmar): máx. 2 refinamentos por linhagem; encerrar uma família após 2 rejeições seguidas; consultas à validação limitadas (≤ 3 no total). Todo experimento conta como trial.
...
- **D10 — Portão de concentração (PENDENTE; exige aprovação).** O portão fixo "top-5 trades ≤ 40% do lucro bruto" é estruturalmente inatingível para arquiteturas de win rate baixo e payoff alto (RUN-0003, Análise A: top-5 típico de subconjuntos aleatórios = 0,84; p5 = 0,70). Proposta: comparar a concentração com o percentil do **placebo de seleção** da própria arquitetura (ou normalizar pelo nº de vencedores) em vez de um limite fixo. Não alterado.
- **D11 — Capacidade condicional de saída no motor (PENDENTE; exige aprovação).** Ativação condicional (breakeven / trailing após +X pontos a favor), mesma semântica conservadora (barras fechadas; nunca na barra de entrada); desbloqueia o EXP-0022 (RUN-0003, BLOCKED). Tarefa separada de motor (05 §10).
