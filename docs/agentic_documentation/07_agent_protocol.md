...
2. **Implementar** só a mudança estrutural necessária, sem tocar o motor de replay. **Revisão point-in-time obrigatória** para cada feature, indicador, filtro ou sinal novo:
   ...
   - rodar os testes de vazamento de [05 §11](05_replay_trading_guide.md) sobre a nova lógica quando tecnicamente praticável: `pytest tests/test_candidates_auto_leakage.py -k <Classe>` (descoberta automática; segundos) antes de aceitar o resultado.
...
- **Orçamento por tempo (D2; diretriz do usuário de 2026-09-19): os X minutos valem SÓ para a experimentação**, isto é, o ciclo de buscar hipóteses (registrar a hipótese → implementar → replay → avaliar → decidir). Ficam **fora do relógio, sem limite de tempo:** o *antes* (git, testes, verificação do baseline, ferramentas, análises preparatórias) e o *depois* (testes de vazamento, pytest completo, relatório, `knowledge.md`, leaderboard, commit e correções decorrentes). Detalhes:
  - **O relógio** começa quando a hipótese do primeiro experimento é registrada e termina quando os X minutos se esgotam. `run.md` e o relatório registram os tempos **separados** (setup, experimentação, fechamento) e o total.
  - **O limite é brando:** ao esgotar o tempo não se **inicia** experimento novo; o que estiver em andamento **pode terminar** (passar um pouco do limite não é problema). Não é preciso reservar tempo de fechamento dentro dos X minutos: usa-se a **janela inteira**, salvo se não restar hipótese independente e não-tuning.
  - **Teto de 25 experimentos por run** (proteção contra excesso e contra inflar o snooping), mesmo sem pressão de tempo. O agente **nunca** busca indefinidamente até achar estratégia lucrativa.
  - Como o tempo não limita o nº de tentativas, **todo experimento conta como trial** e o total (acumulado entre runs) é reportado. "Otimização por X minutos" na Fase 1 significa **buscar arquiteturas dentro do prazo**, não ajustar limiares (Fase 2).
  - **Risco dos testes de vazamento fora do relógio** (o candidato pode ser invalidado depois): mitigado (i) pela **descoberta automática** (`tests/test_candidates_auto_leakage.py` cobre toda classe em `wdo/strategies/candidates*.py` sem teste explícito) e (ii) pela **checagem imediata** de cada candidato assim que é escrito, em segundos: `pytest tests/test_candidates_auto_leakage.py -k <Classe>`. Um resultado só é **aceito** depois dessa checagem; se falhar depois, o candidato é corrigido e reexecutado fora do relógio, sem contar como experimento novo.
...
- **Lições de processo (RUN-0002):** (i) checar cada candidato assim que for escrito com `pytest tests/test_candidates_auto_leakage.py -k <Classe>` (segundos); (ii) o fechamento (relatório, `knowledge.md`, pytest completo, commit) fica **fora do relógio**: usar a **janela inteira** de experimentação, salvo se não houver hipótese independente e não-tuning; (iii) hipóteses independentes podem ser **pré-registradas em lote** (hypothesis.md de todas antes) e rodadas num único script, reportando cada resultado separadamente; (iv) candidato com `stop_price` deve validar o lado protetivo antes de emitir a ordem; (v) placebo com ≥ 20 sorteios quando o custo permitir.
