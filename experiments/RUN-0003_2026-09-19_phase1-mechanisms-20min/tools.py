...
REGISTRY_FIELDS = REGISTRY_FIELDS + ["hypothesis_source", "assumptions_challenged", "reopening_reason", "degrees_of_freedom",
                                     ...
                                     "cumulative_trials"]
PREV_TRIALS = 16         # RUN-0001 (8) + RUN-0002 (8) sobre a mesma partição de pesquisa
...
class Recorder:
    ...
    def on_bar_open(self, bar):
        ...
        for i in intents:
            ...
            if spec is not None and spec.target_price is not None:    # idem para alvo por nível (direção aleatória)
                spec = _replace(spec, target_price=None, target_points=abs(spec.target_price - bar.open))
...
def run_experiment2(exp_id, slug, factory, hypothesis_md, strategy_config, parent_metrics, cfg=None, n_draws=20):
...
def selection_placebo(cand_pnl, base_pnl, n_draw=20000, seed=20260919):
    """P(subconjunto aleatório de mesmo tamanho dos trades da arquitetura-base >= candidato) em PnL médio e PF."""
    cand_pnl, base_pnl = np.asarray(cand_pnl, float), np.asarray(base_pnl, float)
    n = len(cand_pnl)
    def pf(p):
        gl = -p[p < 0].sum()
        return p[p > 0].sum() / gl if gl > 0 else np.inf
    rng = np.random.default_rng(seed)
    means, pfs = [], []
    for _ in range(n_draw):
        s = rng.choice(base_pnl, n, replace=False)
        means.append(s.mean())
        pfs.append(pf(s))
    means, pfs = np.array(means), np.array(pfs)
    return {"n": int(n), "draws": n_draw, "cand_mean": round(float(cand_pnl.mean()), 2), "cand_pf": round(float(pf(cand_pnl)), 3),
            "p_mean": round(float((means >= cand_pnl.mean()).mean()), 3), "p_pf": round(float((pfs >= pf(cand_pnl)).mean()), 3),
            "random_mean_p5_p50_p95": [round(float(x), 1) for x in np.percentile(means, [5, 50, 95])]}


def run_registered(dir_name, factory, strategy_config, parent_metrics, cfg=None, n_draws=20, base_trades_csv=None):
    """Como run_experiment2, mas a pasta e a hipótese já foram registradas ANTES (lote pré-registrado)."""
    import time as _time
    cfg = cfg or CFG
    d = RUN / dir_name
    assert (d / "hypothesis.md").exists(), "registre a hipótese antes de rodar"
    rec = Recorder(factory(cfg))
    result = run_partition_backtest(research_bars(), PARTS, "research", cfg, strategy=rec)
    m = finish_experiment(d, result, strategy_config)
    entries = {t: rec.specs.get(t) for t in result.trades.entry_datetime}
    ...
    if pl:
        m["placebo_excess_expectancy"] = round(m["expectancy"] - pl["mean_expectancy"], 2)
        m["placebo_runtime_s"] = round(_time.time() - t0, 1)
    if base_trades_csv is not None and len(result.trades):
        import pandas as _pd
        base = _pd.read_csv(base_trades_csv)
        if set(_pd.to_datetime(result.trades.entry_datetime, utc=True)) <= set(_pd.to_datetime(base.entry_datetime, utc=True)):
            m["selection_placebo"] = selection_placebo(result.trades.net_pnl.to_numpy(), base.net_pnl.to_numpy())
        else:
            m["selection_placebo"] = "n/a (trades não são subconjunto da arquitetura-base)"
    m["degrees_of_freedom"] = strategy_config.get("dof")
    (d / "metrics.json").write_text(json.dumps(m, indent=1, ensure_ascii=False, default=str), encoding="utf-8")
    base_m = load_metrics(RUN / "EXP-0000_baseline-v0" / "metrics.json")
    print(f"\n===== {dir_name} =====")
    print("V0 (baseline)      :", line(base_m))
    print("PAI                :", line(parent_metrics))
    print("CANDIDATO          :", line(m))
    print("PLACEBO            :", pl, "| excesso da expectância:", m.get("placebo_excess_expectancy"), "| runtime placebo (s):", m.get("placebo_runtime_s"))
    print("SELEÇÃO (placebo)  :", m.get("selection_placebo"))
    for k in ("months_positive", "monthly_pnl", "rolling20_min", "rolling20_pos_frac", "top5_trades_share", "max_month_share", "max_consec_losses",
              "max_days_underwater", "max_drawdown_pct", "worst_trade", "entry_bar_exits", "by_side", "by_entry", "regime_vol_high_next",
              "regime_up_next", "gates", "gates_pass"):
        print(k, "=", json.dumps(m.get(k), ensure_ascii=False))
    print("exit reasons:", result.trades.exit_reason.value_counts().to_dict())
    return d, m
